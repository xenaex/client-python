...
class XenaMDSyncClient(XenaSyncClient):
    ...
    def candles(self, symbol, timeframe='1m', ts_from="", ts_to=""):
        return self._get('/market-data/v2/candles/'+symbol+'/'+timeframe, msg=market_pb2.MarketDataRefresh, params={
            "from": ts_from,
            "to": ts_to,
        })

    def dom(self, symbol, throttling=500, aggregation=0, market_depth=0):
        return self._get('/market-data/v2/dom/'+symbol, msg=market_pb2.MarketDataRefresh, params={
    ...
    def trades(self, symbol, ts_from="", ts_to="", page=1, limit=0):
        return self._get('/market-data/v2/trades/'+symbol, msg=market_pb2.MarketDataRefresh, params={
            "from": ts_from,
            "to": ts_to,
            "page": page,
            "limit": limit
    ...
    def server_time(self):
        resp = self._get('/market-data/v2/server-time', msg=common_pb2.Heartbeat)
...
class XenaTradingSyncClient(XenaSyncClient):
    ...
    def new_order(self, cmd):
        return self._post('/trading/order/new', data=serialization.to_json(cmd))
    ...
    def cancel(self, cmd):
        ...
        return self._post('/trading/order/cancel', data=serialization.to_json(cmd))
    ...
    def mass_cancel(self, account, cancel_id, symbol="", side="", position_effect=constants.PositionEffect_Default):
        ...
        cmd.TransactTime = int(time.time() * 1000000000)
        return self._post('/trading/order/mass-cancel', data=serialization.to_json(cmd))

    def replace(self, cmd):
        if not isinstance(cmd, order_pb2.OrderCancelReplaceRequest):
            raise ValueError("Command has to be OrderCancelRequest")

        return self._post('/trading/order/replace', data=serialization.to_json(cmd))
    ...
    def collapse_positions(self, account, symbol):
        ...
        return self._post('/trading/position/maintenance', data=serialization.to_json(cmd))

    def positions(self, account):
        return self._get('/trading/accounts/' + str(account) + '/positions')
    ...
    def positions_history(self, account, id=0, parentid=0, symbol="", open_ts_from=0, open_ts_to=0, close_ts_from=0, close_ts_to=0, page=1, limit=0):
        return self._get('/trading/accounts/' + str(account) + '/positions-history', params={
            "id": id,
            "parentid": parentid,
            "symbol": symbol,
    ...
    def order(self, account, client_order_id="", order_id=""):
        ...
        return self._get('/trading/accounts/' + str(account) + '/order', params={
    ...
    def active_orders(self, account, symbol=""):
        return self._get('/trading/accounts/' + str(account) + '/active-orders', params={
            "symbol": symbol,
        })

    def last_order_statuses(self, account, symbol="", ts_from=0, ts_to=0, page=1, limit=0):
        return self._get('/trading/accounts/' + str(account) + '/last-order-statuses', params={
            "symbol": symbol,
            "from": ts_from,
            "to": ts_to,
            "page": page,
    ...
    def order_history(self, account, symbol="", client_order_id="", order_id="", ts_from=0, ts_to=0, page=1, limit=0):
        return self._get('/trading/accounts/' + str(account) + '/order-history', params={
            "symbol": symbol,
            "client_order_id": client_order_id,
            "order_id": order_id,
            "from": ts_from,
    ...
    def trade_history(self, account, trade_id="", client_order_id="", symbol="", ts_from=0, ts_to=0, page=1, limit=0):
        return self._get('/trading/accounts/' + str(account) + '/trade-history', params={
            "trade_id": trade_id,
            "client_order_id": client_order_id,
            "symbol": symbol,
            "from": ts_from,
    ...
    def balance(self, account):
        return self._get('/trading/accounts/' + str(account) + '/balance')

    def margin_requirements(self, account):
        return self._get('/trading/accounts/' + str(account) + '/margin-requirements')

    def accounts(self):
        response = self._get('/trading/accounts')
        result = []
    ...
    def heartbeat(self, group_id, interval_in_sec):
        ...
        self._post('/trading/order/heartbeat', data=serialization.to_json(cmd))
